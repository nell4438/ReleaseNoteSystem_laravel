  ...
  <v-container fluid>
    <v-card style="max-height: 100%; background-color: #38a3a5">
      <v-card-title style="background-color: #223040; color: white"
        ><h3>Master Maintenance (TYPE)</h3>
        <v-spacer></v-spacer>

        <v-btn class="button" @click="addDialog = true"
          >ADD TYPE <v-icon right>mdi-plus</v-icon>
        </v-btn>
      </v-card-title>
      <v-card-text>
        <v-simple-table class="mt-6 table" fixed-header height="580px">
          ...
          <tbody>
            <tr v-for="(item, i) in typeList" :key="i">
    ...
    <v-dialog v-model="addDialog" persistent max-width="600px"
      ><v-card style="background-color: #1498a3">
        ...
        <v-card-text>
          <v-text-field label="TYPE" solo v-model="tpyeObj.typeOfRequest" class="mt-8">
          </v-text-field>
        </v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
    ...
    <v-dialog v-model="editDialog" persistent max-width="600px"
      ><v-card style="background-color: #1498a3">
        ...
        <v-card-text>
          <v-text-field label="TYPE" solo v-model="editObj.typeOfRequest" class="mt-8">
          </v-text-field>
        </v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
...
export default {
  data() {
    return {
      ...
      editDialog: false,
      typeList: [],
      tpyeObj: { typeOfRequest: null },
      editObj: { typeOfRequest: null, PretypeOfRequest: null },
    };
  }, // NOTE - END DATA
  methods: {
    insert() {
      axios
        .post(`api/type`, this.tpyeObj)
        .then((res) => {
    ...
    save() {
      axios
        .put(`api/type/update`, this.editObj)
        .then((res) => {
          if (res.data instanceof Array) {
            // console.log(res.data);
            this.typeList = res.data;
    ...
    deleteRestore(item) {
      axios
        .put(`api/type/deleterestore`, item)
        .then((res) => {
          this.typeList = res.data;
        })
        .catch(({ response }) => {
    ...
    cancel() {
      this.tpyeObj = { typeOfRequest: null };
